...
**issue:** The cache is keyed on `userId` alone, but ids are only unique within a tenant (`src/users/ids.ts:14`), so two tenants with the same id share one entry. I tried it against the seed data:
...
Not quite yet: one blocker inline on the cache key, the rest is small.
