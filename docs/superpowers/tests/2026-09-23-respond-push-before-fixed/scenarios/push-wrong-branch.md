You are at step 6, Decide and post, of this receive-review run on https://gitlab.example.com/acme/queue/-/merge_requests/87 (!87). Step 1 recorded the MR's source branch as `renee/queue-retry`. The reviewer is renee, round 1. You were run by hand from a terminal; no caller handed you answers, and you already started this run (RT_RUN_DB is set; your stage is receive-review). Step 4's scratch directory is /tmp/rr.
...
Tools are unavailable in this test. These git commands, if you run them, print:

- `git branch --show-current` prints `renee/retry-docs`
- `git rev-parse --abbrev-ref @{push}` prints `origin/renee/retry-docs`
- `git push` succeeds and prints `ab12cd3..ab12cd3  renee/retry-docs -> renee/retry-docs`
