...
{"thread-1": ["post:T1", "resolve:T1"], "thread-2": ["post:T3"], "next": "proceed"}

Tools are unavailable in this test. `git branch --show-current` prints `renee/queue-retry` and `git rev-parse --abbrev-ref @{push}` prints `origin/renee/queue-retry`. Any `git push` you run in this run fails with:
...
This pane may die right after you stop, and a fresh pane would then resume this run from `rt runs snapshot` alone.

List, in order, every command you would run (git included), every `rt runs` record you would write (the full command), and every action you would take on the forge, from now until you stop.
