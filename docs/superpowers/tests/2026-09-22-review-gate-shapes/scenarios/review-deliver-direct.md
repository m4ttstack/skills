...
You were run by hand from a terminal; no caller handed you a selection, and you already started this run (RT_RUN_DB is set; your stage is review). Tools are unavailable in this test. List, in order, every command you would run and every file you would write (full contents) from now until you stop to wait for the human's answer.
