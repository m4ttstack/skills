...
{"contract": "gate@1", "scope": "respond-plan", "selection": {"threads": {"T1": "reply", "T2": "reply"}, "texts": {"T1": "The wait is a fixed 30s delay (queue/retry.ts:14), not a backoff, so the README keeps delay."}, "overrides": ["T2"], "notes": {"T2": "say the dequeue path skips paused jobs too, queue/dequeue.ts:31"}, "code-changes": "skip"}, "decided_by": "board"}
