...
shows `run.status` = `running` and `run.work_type` = `<verb>` (read each with
...
<verb>` (a new attempt, which re-records this session) and `rt runs field set
hold - --stage <verb>`; re-enter with the snapshot's decisions and do not
