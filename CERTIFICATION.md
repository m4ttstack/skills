...
| 2026-09-23 | mattstack:gate-protocol | pure | pass | n/a (hidden, include-reached) | note scope: "Answers are option values" drops "The note never replaces anything; `text` replaces only what the verb's own act step names" for "What a note or `text` changes is each verb's own act step's call; the protocol swaps neither in for offered text by itself", so a pane note that carries a rename no longer loses to the offered text in every includer; stage-plan-rename (pane composes the answer; strict: renamed line in plan and record, note never `text`, stage finishes) RED 3/10 (6/10 kept the old line) -> control 0.17.23 5/5 -> committed 9/10 (10/10 renamed; the miss sent `text` from the pane, as RED did 1/10); a shorter candidate renamed 5/5 but sent `text` 4/5; stage-provision slug probe RED 0/10, control 5/10, GREEN 4/10 (the residual is stage-provision's own wording); receive-review post-pane-typed and post-act-edited 5/5 strict (docs/superpowers/tests/2026-09-23-gate-protocol-note-scope); certify re-run exit 0 |
