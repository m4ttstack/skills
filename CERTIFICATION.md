...
| 2026-09-23 | mattstack:stage-provision | pure | pass | n/a (hidden) | typed slug: the `slug` question's bullet now says a slug typed in the pane form rides as the note on that answer, with the value left as the option the form offered, and that note is the slug to use; stage-provision-slug (strict: typed slug in the record and `--title`, the pane's answer `{"value":"suggested","note":<slug>}`, `--by pane`) baseline 6/10 on 0.18.0 -> 15/15 with the old gate-protocol (2/15 list-shaped answers) -> 10/10 with the shipped gate-protocol, 0 list-shaped, 0 `text` (docs/superpowers/tests/2026-09-23-followups-slug-pane-decider); certify re-run exit 0 |
| 2026-09-23 | mattstack:gate-protocol | pure | pass | n/a (hidden, include-reached) | pane answer shape: the form branch submits the chosen values "in one object keyed by question id" and shows `{"<question id>": "<value>" \| ["<value>", ...] \| {"value": ..., "note": "..."}}` in place of `'<json>'`; list-shaped pane answers 3/45 before (stage-plan-rename 1/10, post-pane-typed 0/20, slug on the new stage-provision 2/15) -> 0/40 after (10, 20, 10), pane `text` 0 in both; scenario criteria unchanged: stage-plan-rename 10/10 -> 10/10, post-pane-typed 19/20 -> 19/20, post-act-edited 10/10 -> 9/10 (each miss in either arm is a top-level `glab mr note` improvised by an unfilled compile); certify re-run exit 0 |
