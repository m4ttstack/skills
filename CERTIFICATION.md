...
| 2026-09-22 | mattstack:receive-review | pure | pass | n/a (description unchanged) | respond-post per thread: one multi question per offered thread with independent `post:` / `resolve:` options (resolve recommended for fixes only) replaces `replies` plus the blanket `disposition`; per-thread act; decision record `{threads: {<id>: {post, resolve}}}`; gate-ctx.sh validates `reply@1` in place of `replies@1`; 118/118 script tests; RED 0/5 -> GREEN 5/5 on both scenarios (docs/superpowers/tests/2026-09-22-respond-post-per-thread); certify re-run exit 0 |
